defmodule ExMagery do
  ...
  def compile_templates(templates_file_path) do
    trees_list = File.read!(templates_file_path)
    |> Floki.parse
    |> Floki.filter_out(:comment)
    |> fn (ds) -> if is_tuple(ds), do: [ds], else: ds end.()
#    |> Apex.ap

    trees_map = for tree <- trees_list, into: %{} do
  ...
  def transform_floki_boolean_attributes(html_tree) do
...
#    Apex.ap Map.fetch(trees_map, elem(node, 0))
  ...
  defp mgy_to_eex_variables({tag, attributes, children}, trees_map) do
    children = Enum.map(children, fn (c) ->
      cond do
        is_binary(c) ->
    ...
    attributes = Enum.map(attributes, fn (attr) ->
      {k, v} = attr
      cond do
        boolean_attribute?(k) ->
